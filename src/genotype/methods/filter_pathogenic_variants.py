...
def filter_pathogenic_variants(genotype, is_empty):
    """
    input:
    ...
    if not is_empty:
        ...
        genotype.pathogenic_variants = pathogenic_df
    
    else:
        genotype.pathogenic_variants = pd.DataFrame(columns = ['GENE_ID'])
    
    cnvs = genotype.cnvs
    genotype.pathogenic_cnvs = cnvs[cnvs['PATHOGENIC'] > config['cnvoyant_threshold']]
